...
$month_list = $mysqli->query("SELECT\n" .
    ...
    "ORDER BY\n" .
    "MONTH DESC");
$avg_pre = $mysqli->query("select * from team_season where team_name='$team_name' and type='季前赛'")->fetch_row();
$avg_reg = $mysqli->query("select * from team_season where team_name='$team_name' and type='常规赛'")->fetch_row();
$avg_off = $mysqli->query("select * from team_season where team_name='$team_name' and type='季后赛'")->fetch_row();
?>
    ...
    <div class="container">
        <div class="card col-lg-12" id="data">
            <div id="player-data" class="card-header bg-white"><img alt="球队标志" class="float-left img-thumbnail"
                                                                    src="images/team/<?php echo $id ?>.png">
                当前赛季比赛记录
