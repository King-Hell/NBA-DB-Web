...
<body>
		...
		<div class="container">
			<div class="card">
				...
				<div class="card-footer">
					<?php echo"得分：联盟第$avg_reg[19]&emsp;篮板：联盟第$avg_reg[20]&emsp;助攻：联盟第$avg_reg[21]&emsp;抢断：联盟第$avg_reg[22]&emsp;盖帽：联盟第$avg_reg[23]"?>
				</div>
			</div>
		</div>
		<br>
				...
				while ( $row = $month_list->fetch_row() ) {
                    ...
                    <div class=\"card-header\">
                        <a class=\"card-link text-info\" data-toggle=\"collapse\" href=\"#collapse-$row[0]\">
                            <h5><i class=\"icon-circle-down\"></i>&nbsp;$name&nbsp;$row[0]月数据&nbsp;<span class=\"badge badge-secondary\">$row[1]场比赛</span>&nbsp;<i class=\"icon-circle-down\"></i></h5>
                        </a>
                    </div>
	...
	<div id="btn-compare" class="card text-center">
		...
		</div><a class="btn btn-outline-info" href="compare.php">对比</a>
	</div>
	<div class="container">
		<div class="row">
	...
	<script>
		...
		$( document ).ready( cookie_init() );
		$( '#player1_add' ).click( function () {
			$.cookie( 'player1_id', '<?php echo $id?>', {path: '/'} );
			$.cookie( 'player1_name', '<?php echo $result['name']?>', {path: '/'} );
			cookie_init();
		} );
		$( '#player2_add' ).click( function () {
			$.cookie( 'player2_id', '<?php echo $id?>', {path: '/'} );
			$.cookie( 'player2_name', '<?php echo $result['name']?>', {path: '/'} );
			cookie_init();
		} );
		...
		$( '#btn-all' ).click( function () {
			if ( $( this ).attr( 'data-span' ) == "false" ) {
				...
				$( this ).text( "折叠全部" )
				$( 'i' ).addClass( 'icon-circle-up' )
			} else {
				$( '.col-player' ).collapse( 'hide' )
				$( this ).attr( "data-span", "false" )
				$( this ).text( "展开全部" )
				$( 'i' ).removeClass( 'icon-circle-up' )
			}
	...
	<script>
		document.cookie = "player1=John Doe;path=/";
